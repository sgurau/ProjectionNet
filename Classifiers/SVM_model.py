...
def svm_cross_validation_and_hyperparameter_tuning(features, labels):

    def objective(trial):
        # Hyperparameter values
        kernel = trial.suggest_categorical('kernel', ['linear', 'rbf', 'sigmoid']) # exclude 'linear' for split 24
        c = trial.suggest_float('C', 1e-4, 1e3, log=True) 
        gamma = trial.suggest_float('gamma', 1e-5, 1e-2, log=True)
        model = SVC(C=c, gamma=gamma, kernel=kernel, probability=True, class_weight='balanced')
    ...
    # Training final model on the training dataset with the best parameters
    best_model = SVC(C=best_params['C'], gamma=best_params['gamma'], kernel=best_params['kernel'], probability=True, class_weight='balanced')
    ...
    # avg_roc_auc = np.mean([roc_auc_score(labels[test_idx], best_model.predict_proba(features[test_idx])[:, 1]) for test_idx in skf.split(features, labels)])
    # avg_accuracy = np.mean([accuracy_score(labels[test_idx], best_model.predict(features[test_idx])) for test_idx in skf.split(features, labels)])
      
    return best_model, best_params, train_accuracy, val_accuracy, mean_roc_auc # X_train, X_val, y_train, y_val avg_accuracy, roc_auc_val
        ...
        if filename.startswith(file_prefix + 'train_') and filename.endswith('.h5'):
            ...
            if indices_to_process is None or file_index in indices_to_process:
                ...
                (
                    ...
                    val_accuracy,
                    mean_roc_auc
                    ) = svm_cross_validation_and_hyperparameter_tuning(features, labels)
...
if __name__ == "__main__":
    ...
    indices_to_process = [4] # range(15, 22)  # Specify as needed
