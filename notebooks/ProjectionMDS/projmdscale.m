function [Y,stress,disparities] = projmdscale(D,p,varargin)
...
for rep = 1:nreps
    ...
    if metric
        ...
        [Y,stress,~,disparities] = MDS(D,P0,dissimilarities,weights,stressFun,metric,weighted,options);
    end
...
function [P,stress,iter,disparities] = ...
             MDS(D,P,dissimilarities,weights,stressFun,metric,weighted,options)
Y = D*P; 
n = size(Y,1);
...
while true
    ...
    [stress,grad] = feval(stressFun, D,P, disparities, weights);
    normGrad = norm(grad(:));
    ...
    maxStepLen = 2;
    while true
        stress = lineSearchCrit(maxStepLen,D,P,stepDir,disparities,weights,stressFun);
    ...
    [alpha, stress, err, output] = ...
            fminbnd(@lineSearchCrit,0,maxStepLen,lineSearchOpts, ...
                    D,P,stepDir,disparities,weights,stressFun);
    if (err == 0)
...
function val = lineSearchCrit(t,D,P,stepDir,disparities,weights,stressFun)
...
val = feval(stressFun,D,P+t*stepDir,disparities,weights);


%==========================================================================

function [S,grad] = stressCrit(D,P,disparities,weights)
